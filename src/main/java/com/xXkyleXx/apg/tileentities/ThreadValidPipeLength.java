...
public class ThreadValidPipeLength extends Thread {
	...
	public void run() {
		...
		for (int i = 1; i < pipeLength; i++) {
			if (pump.yCoord - i < 81 && getAirBlocksInLayer(pump.xCoord, pump.yCoord - i, pump.zCoord, 3) < 3 && getAirBlocksInLayer(pump.xCoord, pump.yCoord - i, pump.zCoord, pump.getSearchSize()) < Math.pow(pump.getSearchSize(), 2) / 3) {
				length++;
			}
